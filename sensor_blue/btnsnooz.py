...
def decode_snooz(snooz):
  ...
  """
    ...
    outf.write(b'btsnoop\x00\x00\x00\x00\x01\x00\x00\x03\xea')
    if version == 1:
        decode_snooz_v1(decompressed, last_timestamp_us)
    elif version == 2:
        decode_snooz_v2(decompressed, last_timestamp_us)
...
def decode_snooz_v1(decompressed, last_timestamp_ms):
  ...
  """
    ...
    # Second pass does the actual writing.
    offset = 0
    while offset < len(decompressed):
        length, delta_time_ms, type = struct.unpack_from('=HIb', decompressed, offset)
        first_timestamp_ms += delta_time_ms
        offset += 7
        outf.write(struct.pack('>II', length, length))
        outf.write(struct.pack('>II', type_to_direction(type), 0))
        outf.write(struct.pack('>II', (first_timestamp_ms >> 32), (first_timestamp_ms & 0xFFFFFFFF)))
        outf.write(type_to_hci(type))
        outf.write(decompressed[offset:offset + length - 1])
        offset += length - 1
...
def decode_snooz_v2(decompressed, last_timestamp_ms):
  ...
  """
    ...
    while offset < len(decompressed):
        ...
        first_timestamp_ms += delta_time_ms
        offset += 9
        outf.write(struct.pack('>II', packet_length, length))
        outf.write(struct.pack('>II', type_to_direction(snooz_type), 0))
        outf.write(struct.pack('>II', first_timestamp_ms >> 32, (first_timestamp_ms & 0xFFFFFFFF)))
        outf.write(type_to_hci(snooz_type))
        outf.write(decompressed[offset:offset + length - 1])
        offset += length - 1

def main():
    if len(sys.argv) != 3:
        sys.stderr.write('Usage: %s [bugreport] [outfile]\n' % sys.argv[0])
        exit(1)
    iterator = fileinput.input(files=sys.argv[1])
    global outf
